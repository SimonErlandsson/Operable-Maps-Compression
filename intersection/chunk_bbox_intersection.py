...
chunk_to_shape = lambda chk: shapely.Point(chk[0]) if len(chk) == 1 else shapely.LineString(chk)
...
def is_contained_within(containee, container):
    ...
    #### DEBUG
    if not PLOT_ONLY_INCORRECT or debug_correct_ans != None and debug_correct_ans != (len(intersecting_points) % 2 == 1):
...
def has_line_intersection(bins, bbox):
    chks = [[], []]
    segments = [[], []]
    for i in range(2):
    ...
    for i in segments[0]:
        for j in segments[1]:
            if i.intersects(j) and PLOT_ONLY_INCORRECT and debug_correct_ans == None or debug_correct_ans == True:
                ...
                if not PLOT_ONLY_INCORRECT or debug_correct_ans != None and debug_correct_ans != (len(intersecting_points) > 0):
                    for cs in segments[0] + segments[1]:
                        plot_geometry(cs)
                    plot_intersecting_points(intersecting_points)
                    plot_chunks_bounds(bins[0], include_next_chunk_start=True, avoid_show=True, idxs=chks[0], txt=debug_correct_ans)
                    plot_chunks_bounds(bins[1], include_next_chunk_start=True, avoid_create_frame=True, idxs=chks[1], txt=debug_correct_ans)
                ####### END
                return True
    return False

debug_correct_ans = False

def is_intersecting(bins, debug_correct_ans_in):
    debug_correct_ans = debug_correct_ans_in
    ...
    intersects = has_line_intersection(bins, bbox)
    ...
    if not intersects and overlap_type == '1 in 2':
        intersects = is_contained_within(bins[0], bins[1])
    elif not intersects and overlap_type == '2 in 1':
        intersects = is_contained_within(bins[1], bins[0])
    return intersects

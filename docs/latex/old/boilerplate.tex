\documentclass[nofilelist]{../scripts/cslthse-msc}
...
\title{Compression Algorithms for Geometries Supporting Basic Operations}
\svensktitel{Kompressionsalgoritmer för geometrier med stöd för grundläggande operationer}
...
\subsubsection{Since 2020/09/18}
\begin{itemize}
\item Added the Swedish title displayed on page 3. Use \verb+\svensktitel{...}+ command to set it.
\item Converted the default LU-CS-EX number to a magic number, \verb+2023-79+, easy to find and change in PDF with \verb+changepagestring.pl+; this is a \verb+perl+ command line utility found in \verb+CAM::PDF+ module -- but only works on some PDFs (e.g. generated with \verb+pdflatex+) but not others (e.g. \verb+xelatex+) \textbf{[CS internal use]}
\end{itemize}

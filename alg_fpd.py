...
# ---- HELPER METHODS
    ...
    def init_bytearray(self, geometry, delta_size, chk_size):
        ...
        res.append(self.int_to_bytes(chk_size))
        res.append(self.type_int_convertion[geometry.geom_type].to_bytes(
            1, 'big'))  # 1 byte is enought for storing type
        return res
    ...
    def fp_delta_encoding(self, geometry, max_chk_size, delta_size):
        ...
        res = self.init_bytearray(geometry, delta_size, max_chk_size)
        ...
        for i in range(len(coords)):
            ...
            if chk_point_nbr == 0:
                ...
                chk_size_idx = self.init_chunk(
                    res, max_chk_size, coords, i, chk_ys, chk_xs)
                chk_point_nbr += 1

            else:  # Loop for delta
                zig_delta_x = self.get_zz_encoded_delta(
                    coords[i - 1][0], coords[i][0])
                zig_delta_y = self.get_zz_encoded_delta(
                    coords[i - 1][1], coords[i][1])

                if self.deltas_fit_in_bytes(delta_size, zig_delta_x, zig_delta_y):
                    self.append_delta_pair(
                        chk_xs, chk_ys, zig_delta_x, zig_delta_y, delta_size)
                    chk_point_nbr += 1

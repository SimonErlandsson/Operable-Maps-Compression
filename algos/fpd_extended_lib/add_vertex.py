...
def add_vertex(self, args):
    def create_chunk(reset_point, delta_cnt=0):
        ...
        middle.extend(uint_to_ba(0, D_CNT_SIZE))
    ...
    while (p_idx <= insert_idx):
        ...
        if p_idx <= insert_idx and insert_idx <= p_idx + deltas_in_chunk + (1 if chunks_in_ring_left == 1 else 0):
            ...
            # Get the absolute coordinate for first right coordinate, not calculated in p_idx != insert_idx because need for bytes offset
            rst_p, _, delta_bytes_offset = access_vertex_chk(bin, deltas_in_chunk_offset, delta_size, insert_idx - p_idx, get_delta_offsets=True)
            ...
            if p_idx != insert_idx:  # Has a left coordinate?
                split = deltas_in_chunk_offset + D_CNT_SIZE + FLOAT_SIZE * 2 + delta_bytes_offset[deltas_left] - delta_bytes_offset[0]
                # Update delta cnt

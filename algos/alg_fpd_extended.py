...
# ---- UNARY ---- #
    ...
    def add_vertex(self, args):
        def create_chunk(reset_point, delta_cnt=0):
            ...
            middle.extend(self.uint_to_ba(delta_cnt, self.D_CNT_SIZE))
            # Add full coordinates
        ...
        while (p_idx <= insert_idx):
            ...
            if p_idx <= insert_idx and insert_idx <= p_idx + deltas_in_chunk + (1 if chunks_in_ring_left == 1 else 0):
                ...
                if p_idx != insert_idx:  # Has a left coordinate?
                    split = deltas_in_chunk_offset + self.D_CNT_SIZE + 64 * 2 + delta_size * 2 * deltas_left
                    # Update delta cnt
                    bin[deltas_in_chunk_offset:deltas_in_chunk_offset + self.D_CNT_SIZE] = self.uint_to_ba(deltas_left, self.D_CNT_SIZE)
                else:
                ...
                left = bin[0:split]
                if not is_linestring:
                    left[chunks_in_ring_offset:chunks_in_ring_offset + self.RING_CHK_CNT_SIZE] = self.uint_to_ba(chunks_in_ring, self.RING_CHK_CNT_SIZE)
                bin = left
                bin.extend(middle)
                bin.extend(right)
